...
public:
  ...
  FunctionStmt(Token name, const std::vector<Token>& args,
...
class Parser {
  ...
  std::shared_ptr<FunctionStmt> function(string kind) {
    ...
    std::vector<std::shared_ptr<Stmt>> body = block(false);
    return std::make_shared<FunctionStmt>(name, args, body);
  }
...
public:
  ...
  virtual Value visitFunctionStmt(FunctionStmt* stmt) override {}
